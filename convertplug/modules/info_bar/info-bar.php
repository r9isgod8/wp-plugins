				...
				if ( is_array( $live_styles ) && ! empty( $live_styles ) ) {
					...
					foreach ( $info_bar_arrays as $key => $info_bar_array ) {
						...
						if ( $display ) {
							...
							echo '<link rel="stylesheet" id="' . $id . '" href="' . esc_url( $url ) . '" type="text/css" media="all" />';
	...
	function cp_info_bar_custom( $atts, $content = null ) {
		...
		foreach ( $live_styles as $key => $info_bar_array ) {
			...
			if ( $id === $style_id ) {
				...
				$encode_settings  = json_encode( $style_settings );
				...
				if ( $display ) {
					...
					echo '<link rel="stylesheet" id="' . esc_attr( $id ) . '" href="' . esc_url( $url ) . '" type="text/css" media="all" />';

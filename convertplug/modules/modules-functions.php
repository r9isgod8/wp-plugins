...
function cp_is_style_visible( $settings ) {
	...
	if ( $show_module && $show_countries ) {
		...
		$temp = array();
		if ( '' !== $exclusive_on && is_array( $exclusive_on ) ) {
			foreach ( $exclusive_on  as $key => $value ) {
				$test = explode( '::', $value );

				$temp[ $key ]['id']   = $test[0];
				$temp[ $key ]['text'] = $test[1];
			}
		...
		if ( $global_display ) {
			...
			if ( is_archive() ) {
				...
				if ( is_array( $exclusive_on ) ) {
					foreach ( $exclusive_on as $key1 ) {
						$exclusive_on_id  = $key1['id'];
						$exclusive_on_key = $key1['text'];
						$temp             = get_cat_name( $term_id );

						if ( $exclusive_on_id == $term_id && $exclusive_on_key == $temp ) {
							$display = true;
						}
					}

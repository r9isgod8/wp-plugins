...
class WP_Optimization_images extends WP_Optimization {
	...
	public function get_unused_images_in_sub_directory($sub_directory, $blog_id = 1) {
		...
		if ($handle = opendir($base_upload_dir.'/'.$sub_directory)) {
			...
			while (false !== $file) {
				...
				// check if file name is for resized image then get original filename for image.
				if (preg_match('/^(.+)\-(\d+)x(\d+)(\@\dx)?(\.\w+)$/', $image_file_name, $parts)) {
					$original_file_name = $parts[1].$parts[5];
				} elseif (preg_match('/^(.+)\-updraft\-pre\-smush\-original(\.\w+)$/', $image_file_name, $parts)) {
					// if this is smush backup.
	...
	/**
	 * Check if image filename is for resized image.
	 *
	 * @param string $filename filename.
	 * @return mixed
	 */
	public function is_resized_image($filename) {
		return preg_match('/^.+\-\d+x\d+(\@\dx)?\.\w{3,4}+$/', $filename);
	}
	...
	public function get_original_image_file_name($filename) {
		if (preg_match('/^(.+)\-\d+x\d+(\@\dx)?(\.\w+)$/', $filename, $parts)) {

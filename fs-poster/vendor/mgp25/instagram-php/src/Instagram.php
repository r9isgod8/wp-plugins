...
class Instagram implements ExperimentsInterface
{
	...
	public function challengeApprove(
		$username ,
		$password ,
		$code ,
		$user_id ,
	...
	)
	{
		...
		$endpiont = 'challenge/' . $user_id . '/' . $nonce_code . '/';

		return $this->_challengeApprove($username, $password, $code , $endpiont);
	}
        ...
        if (!$this->isMaybeLoggedIn || $forceLogin) {
            ...
            try {
            	...
            	if ($e->hasResponse() && $e->getResponse()->isChallenge()) {
		            ...
		            $result = $e->getResponse()->asArray();
		            $crURL = str_replace('https://i.instagram.com/' , '' , $result['challenge']['url']);

		            try
		            {
			            $acceptChallengeRequest = $this->request($crURL)
			                                           ->setNeedsAuth(false)
			                                           ->addPost('choice', '1')
			                                           ->addPost('_csrftoken', $this->client->getToken())
			                                           ->addPost('username', $this->username)
		            ...
		            {
			            if( $e->hasResponse() &&
			                ($response1 = $e->getResponse()->asArray()) &&
			                isset($response1['status']) && $response1['status'] == 'fail' &&
			                isset($response1['message']) && strpos($response1['message'] , 'valid choice') !== false )
			            {
				            try
				            {
					            $acceptChallengeRequest = $this->request($crURL)
	            ...
	            /*else if ($e->hasResponse() && $e->getResponse()->isCheckpointUrl()) {

	            }*/
                else if ($e->hasResponse() && $e->getResponse()->isTwoFactorRequired()) {
	...
	protected function _challengeApprove(
		$username,
		$password,
		$code,
		$endpoint
	)
	{
		...
		{
			throw new Exception\LoginRequiredException(' Error! Please login instagram and compleate challenge!');
		}

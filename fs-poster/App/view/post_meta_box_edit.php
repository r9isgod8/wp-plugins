...
if( isset($_GET['share']) && !empty($_GET['share']) && $_GET['share'] == '1' )
{
	...
	{
		$chechNotSendedFeeds = DB::DB()->get_row(DB::DB()->prepare("SELECT count(0) AS cc FROM ".DB::table('feeds')." WHERE post_id=%d AND is_sended=0" , [(int)$parameters['post']->ID]) , ARRAY_A);
	}
}

$feeds = DB::fetchAll('feeds' , ['post_id' => $parameters['post']->ID]);
?>
...
<div>
	<div id="share_box1_edit">
		...
		<div class="share_box_items_edit">
			...
			foreach ($feeds AS $feedInf)
			{
				...
				<div class="share_box_node_edit">
					<div class="node_img"><img src="<?=Helper::profilePic($nodeInf)?>" onerror="$(this).attr('src', '<?=Helper::assets('images/no-photo.png')?>');"></div>
					<div class="node_label" style="width: 100%;">
						<div><a href="<?=Helper::profileLink($nodeInf)?>" target="_blank"><?=esc_html($nodeInf['name']);?></a></div>
						<div class="node_label_help"><?=esc_html(ucfirst($nodeInf['driver']) . ' > ' . $nodeInf['node_type'] . ( $feedInf['feed_type'] == 'story' ? ' > Story' : '' ));?></div>
					</div>
					<div class="node_actions">
						...
						if( $feedInf['status'] == 'ok' )
						{
							...
							<a href="<?=Helper::postLink($feedInf['driver_post_id'] , $feedInf['driver'] , $username , $feedInf['feed_type'])?>" target="_blank" class="post_link" title="<?=esc_html__('Open post in facebook!' , 'fs-poster')?>"><i class="fa fa-external-link fa-external-link-alt"></i></a>
							<?php
						}
						else if( $feedInf['is_sended'] == '0' && get_post_status($parameters['post']->ID) == 'future' )
						{
							?>
		...
		<?php
		if( get_post_status($parameters['post']->ID) == 'publish' )
		{
			?>
			<div style="margin-top: 10px; display: flex; justify-content: space-between;">
				<button type="button" class="button button-primary" style="width: 49%;" data-load-modal="share_saved_post" data-parameter-post_id="<?=$parameters['post']->ID?>"><?=empty($feeds) ? 'Share' : 'Share again'?></button>
				<button type="button" class="button button-secondary" style="width: 49%; background: #e84393; color: #FFF; border: 0;" data-load-modal="plan_saved_post" data-parameter-post_id="<?=$parameters['post']->ID?>">Schedule</button>
			</div>
			<?php
		}
		?>
...
<script>
	jQuery(document).ready(function()
	{
		...
		{
			?>
			fsCode.loadModal('share_feeds' , {'post_id': '<?=(int)$parameters['post']->ID?>'})
			<?php
		}
		?>
	});

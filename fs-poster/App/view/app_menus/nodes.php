...
<div style="display: flex; margin-left: 50px; margin-top: 15px; align-items: center;">
	...
	<div style="width: 220px;">
		<select class="ws_form_element" id="accountFilterSelect">
			...
			<?php
			$accounts = DB::DB()->get_results( DB::DB()->prepare("SELECT * FROM " . DB::table('accounts') . " WHERE user_id=%d AND driver=%s" , [get_current_user_id() , $tab]) , ARRAY_A );
						...
						<div class="node_label">
							<div>
								<div class="node_label_title">
									<a href="<?=Helper::profileLink($node)?>" target="_blank" title="<?=esc_html__('Profile link' , 'fs-poster');?>"><?=esc_html($node['name']);?></a>
								</div>

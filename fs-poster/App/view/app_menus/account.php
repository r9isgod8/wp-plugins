...
$accountsList = DB::DB()->get_results(DB::DB()->prepare("SELECT driver, COUNT(0) AS _count FROM ".DB::table('accounts')." WHERE (user_id=%d OR is_public=1) GROUP BY driver",  [get_current_user_id()]) , ARRAY_A);
$accountsCount = [
	'fb'		=>  0,
	'twitter'	=>  0,
	'instagram'	=>  0,
...
<div id="fs_sub_menu">
	...
	<div class="activate_with_condition_btn">Activate (condition)</div>
	<div class="deactivate_btn">Deactivate</div>
</div>

<div id="fs_sub_menu2">
	<div class="make_public_action" data-type="public">Make public <i class="fa fa-question-circle ws_tooltip" data-title="<?=__('Using this feature you will be able to allow other WordPress Users to share in this profile.' , 'fs-poster')?>"></i></div>
	<div class="make_public_action" style="display: none;" data-type="private">Make private <i class="fa fa-question-circle ws_tooltip" data-title="<?=__('Using this feature you will be able to allow other WordPress Users to share in this profile.' , 'fs-poster')?>"></i></div>
...
<script>
	jQuery(document).ready(function()
	{
		...
		}).on('click', '.more_options_account', function ()
		{
			...
			if( $(this).closest('tr').find('.fs_account_is_public').css('display') == 'none' )
			{
				$('#fs_sub_menu2 > [data-type="public"]').show();
				$('#fs_sub_menu2 > [data-type="private"]').hide();
			}
			else
			{
				$('#fs_sub_menu2 > [data-type="private"]').show();
				$('#fs_sub_menu2 > [data-type="public"]').hide();
			}

...
$schedules = DB::DB()->get_results(
	DB::DB()->prepare( 'SELECT *, (SELECT COUNT(0) FROM `' . DB::table('feeds') . '` WHERE `schedule_id`=tb1.id) AS `shares_count` FROM `' . DB::table('schedules') . '` tb1 WHERE `user_id`=%d', [get_current_user_id()] ),
	ARRAY_A
);
...
<div class="fs_warning_box">
	Web4Sync | If Scheduled posts do not work properly, please read this instruction: <a href="https://www.fs-poster.com/doc/configure-cron-job-tasks-on-wordpress" target="_blank">Configure Cron Job</a>
</div>
			...
			<tr data-id="<?=$scheduleInf['id']?>">
				<td>
					...
					<div style="display: inline-block; vertical-align: middle">
						<div>
							<?php
							print esc_html(Helper::cutText($scheduleInf['title'], 55));
						...
						<div style="font-size: 11px; font-weight: 500; color: #999; margin-right: 25px;">
							...
							if( count( $postIds ) == 1 )
							{
								print 'Post ID: ' . reset( $postIds ) . ' ( <a href="' . esc_html(get_permalink( reset( $postIds ) )) . '" target="_blank">' . esc_html( Helper::cutText( get_the_title( reset( $postIds ) ) ) ) . '</a> )';
							}
							else
							{
...
<script src="<?=Helper::assets('js/jquery-ui.js')?>"></script>
<link rel="stylesheet" href="<?=Helper::assets('css/jquery-ui.css')?>">
<script>

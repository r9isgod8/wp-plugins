...
?>
<link rel="stylesheet" href="<?=Helper::assets('css/fullcalendar.min.css')?>">
		...
		fsCode.ajax('schedule_get_calendar' , {'month': _month+1 , 'year': _year}, function(result)
		{
			...
			for( var date in result['days'] )
			{
				...
				$(".plan_posts_list").append(
					...
					'</div>' +
					'<span class="remove_plan ws_tooltip" data-title="Remove plan" data-float="left"><i class="fa fa-trash fa-trash-o"></i></span>' +
					'</div>');
			}

...
$postId = (int)Request::get('post_id' , '0' , 'int');

DB::DB()->query("DELETE FROM " . DB::DB()->base_prefix . "posts WHERE post_type='fs_post_tmp' AND id<>'{$postId}' AND CAST(post_date AS DATE)<CAST(NOW() AS DATE)");
			...
			<div>
				<table class="ws_table">
					...
					<tbody>
						...
						$posts = DB::DB()->get_results('SELECT * FROM ' . DB::DB()->base_prefix . "posts WHERE post_type='fs_post' AND post_author='" . $currentUserId ."' ORDER BY ID DESC", ARRAY_A);

						foreach( $posts AS $post )
						{
							print '<tr data-id="' . (int)$post['ID'] . '">';
							print '<td>' . (int)$post['ID'] . '</td>';
							print '<td><a href="?page=fs-poster-share&post_id=' . (int)$post['ID'] . '" style="text-decoration: none;">[ ' . htmlspecialchars(Helper::cutText($post['post_content'])) . ' ]</a></td>';
							print '<td>
...
<script>
	jQuery(function($)
	{
		...
		frame.on( 'select', function()
		{
			...
			$("#wpMediaBtn , .link_url").slideUp(200);
		});
		...
		{
			$("#imageShow")
				.slideUp(200, function(){ $(this).children('img').attr('src' , ''); })
				.data('id' , 0);
			$("#wpMediaBtn , .link_url").slideDown(200);
		});

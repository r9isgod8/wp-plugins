...
$publicCommunities = DB::DB()->get_results(DB::DB()->prepare("
	...
	WHERE driver='tumblr' AND (user_id=%d OR is_public=1) AND account_id NOT IN ('" . implode("','", $collectMyAccountIDs) . "')",  [get_current_user_id() , get_current_user_id() ]) , ARRAY_A);
...
<div class="fs_accounts_m_header">
	...
	<span class="fs_accounts_count"><span id="accounts_count"><?php print count($accountsList);?></span> <?=esc_html__('Tumblr account added', 'fs-poster')?></span>
		...
		<table class="ws_table">
			...
			<tr data-id="<?=(int)$accountInf['id']?>" data-type="account">
				...
				<td>
					...
					<span class="fs_communities_count"><?=$accountInf['blogs']?> BLOGS</span>
				</td>
				<td style="width: 15%;">

...
$publicCommunities = DB::DB()->get_results(DB::DB()->prepare("
	...
	WHERE driver='vk' AND (user_id=%d OR is_public=1) AND account_id NOT IN ('" . implode("','", $collectMyAccountIDs) . "')",  [get_current_user_id() , get_current_user_id() ]) , ARRAY_A);
...
<div class="fs_accounts_m_header">
	...
	<span class="fs_accounts_count"><span id="accounts_count"><?php print count($accountsList);?></span> <?=esc_html__('vk account added', 'fs-poster')?></span>
			...
			<tbody>
				<tr data-id="<?=$accountInf['id']?>" data-type="account">
					...
					<td>
						...
						<span class="fs_communities_count"><?=$accountInf['communities']?> COMMUNITIES</span>
					</td>
					<td style="width: 15%;">
						...
						<tr data-id="<?=$nodeInf['id']?>" data-type="community">
							<td>
								...
								<span class="fs_community_categ">
									...
									<?=ucfirst(esc_html($nodeInf['node_type'] ) ) . ($nodeInf['category'] == 'admin' ? ' (admin)' : '')?>
								</span>
							</td>
							<td style="width: 15%;">

...
$accountsList = DB::DB()->get_results(DB::DB()->prepare("
	...
	WHERE (user_id=%d OR is_public=1) AND driver='medium'",  [get_current_user_id(), get_current_user_id(), get_current_user_id()]) , ARRAY_A);
...
$publicCommunities = DB::DB()->get_results(DB::DB()->prepare("
	...
	WHERE driver='medium' AND (user_id=%d OR is_public=1) AND account_id NOT IN ('" . implode("','", $collectMyAccountIDs) . "')",  [get_current_user_id() , get_current_user_id() ]) , ARRAY_A);
...
<div class="fs_accounts_m_header">
	...
	<span class="fs_accounts_count"><span id="accounts_count"><?php print count($accountsList);?></span> <?=esc_html__('Medium account added', 'fs-poster')?></span>
	<button type="button" class="ws_btn ws_bg_dark" style="float: right;" data-load-modal="add_medium_account"><i class="fa fa-plus"></i> <?=esc_html__('ADD ACCOUNT', 'fs-poster')?></button>
</div>
		...
		<table class="ws_table">
			...
			<tr data-id="<?=(int)$accountInf['id']?>" data-type="account">
				...
				<td>
					...
					<span class="fs_communities_count"><?=$accountInf['publications']?> PUBLICATIONS</span>
				</td>
				<td style="width: 15%;">

...
{
	$chechNotSendedFeeds = DB::DB()->get_row(DB::DB()->prepare("SELECT count(0) AS cc FROM ".DB::table('feeds')." WHERE post_id=%d AND is_sended=0" , [(int)$postId]) , ARRAY_A);
}
...
{
	$shareCheckbox						= get_option('fs_auto_share_new_posts', '1') || Request::get('page')=='fs-poster-share' || Request::post('post_id', null) !== null;
				...
				<div class="fs_share_box_node" data-tab="<?=$nodeInf['driver']?>">
					...
					<div class="node_label" style="width: 100%;">
						<div>
							...
							<a href="<?=Helper::profileLink($nodeInf)?>" target="_blank" class="ws_btn ws_tooltip" data-title="Profile link" style="font-size: 13px; color: #fd79a8;"><i class="fa fa-external-link fa-external-link-alt"></i></a>
						</div>
						<div class="node_label_help"><i class="<?=Helper::socialIcon($nodeInf['driver'])?>"></i> <?=ucfirst($nodeInf['driver'])?> <i class="fa fa-chevron-right " style="font-size: 10px; color: #CCC;"></i> <?=esc_html($nodeInf['node_type']);?> <?=empty($titleText) ? '' : '<i class="fa fa-filter" title="'.$titleText.'" style="padding-left: 5px; color: #fdcb6e;"></i>'?></div>
					</div>

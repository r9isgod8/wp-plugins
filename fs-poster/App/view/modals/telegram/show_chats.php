	...
	<table class="ws_table" id="telegram-chat-list-tbl" style="margin-bottom: 40px; ">
		...
		foreach( $nodeList AS $node )
		{
			...
			<tr data-id="<?=(int)$node['id']?>">
				<td>
					<img class="ws_img_style" src="<?=Helper::profilePic($node)?>"><span style="vertical-align: middle;"><?=htmlspecialchars($node['name'])?></span>
					<a href="<?=Helper::profileLink($node)?>" target="_blank" class="ws_btn ws_tooltip" data-title="Chat link" style="font-size: 13px; color: #fd79a8;"><i class="fa fa-external-link fa-external-link-alt"></i></a>
				</td>

...
$accountsList = DB::DB()->get_results(
	DB::DB()->prepare('SELECT *, \'account\' AS node_type, \'account\' AS category FROM ' . DB::table('accounts') . " WHERE (user_id=%d OR is_public=1) AND driver<>'tumblr' ORDER BY driver", [get_current_user_id()])
	, ARRAY_A
);

$pagesList = DB::DB()->get_results(
	DB::DB()->prepare('SELECT * FROM ' . DB::table('account_nodes') . " WHERE (user_id=%d OR is_public=1) ORDER BY node_type", [get_current_user_id()])
	, ARRAY_A
);
	...
	<div class="nodes_list">
		...
		function printNodeCart( $node, $isSub = false )
		{
			...
			<div class="node_div<?=$isSub?>" data-id="<?=$val?>">
				<div class="node_img"><img src="<?=Helper::profilePic($node);?>" onerror="$(this).attr('src', '<?=Helper::assets('images/no-photo.png')?>');"></div>

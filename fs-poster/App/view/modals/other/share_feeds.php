...
<style>
	...
	.popup_background
	{
		background-image: url('<?=Helper::assets('images/modal_bg1.png')?>');
...
<span class="close" data-modal-close="true" style="display: none; z-index: 9; color: #FFF;" onclick="<?=isset($parameters['dont_reload'])?'':'location.reload();'?>">&times;</span>
		...
		<table class="feeds_table" id="share_table">
			...
			foreach ($parameters['feeds'] AS $feedInf)
			{
				...
				<tr data-id="<?=(int)$feedInf['id']?>" data-interval="<?=(int)$feedInf['interval']?>" data-status="<?=(int)$feedInf['is_sended']?>">
					<td class="node_name">
						<div><img class="ws_img_style" src="<?=Helper::profilePic($nodeInf)?>" onerror="$(this).attr('src', '<?=Helper::assets('images/no-photo.png')?>');"></div>
						<div>
							<div style="color: #888;">
					...
					<td class="link_td">
						...
						{
							?>
							<a href="<?=Helper::postLink($nodeInf['driver_post_id'] , (isset($nodeInf['driver']) ? $nodeInf['driver'] : 'fb') , $username, $feedInf['feed_type'])?>" target="_blank"><i class="fa fa-external-link fa-external-link-alt"></i> <?=esc_html__('Post link' , 'fs-poster')?></a>
						<?php
						}
						?>
					</td>

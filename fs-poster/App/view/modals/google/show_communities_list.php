...
$nodeList = DB::DB()->get_results(DB::DB()->prepare("
	SELECT 
		*,
		(SELECT filter_type FROM ".DB::table('account_node_status')." WHERE node_id=tb1.id AND user_id=tb1.user_id) is_active
...
<div class="m2header">Communities list</div>

<div style="margin-top: 10px; padding: 20px; max-height: 260px; overflow: auto;">
	<table class="ws_table" id="google-communities-list-tbl" style="margin-bottom: 40px; ">
		<thead>
		<tr>
			<th>NAME</th>
			<th>PICK A CATEGORY</th>
		...
		foreach( $nodeList AS $node )
		{
			...
			<tr data-id="<?=(int)$node['id']?>">
				<td>
					<img class="ws_img_style" src="<?=htmlspecialchars($node['cover'])?>">
					<span style="vertical-align: middle;"><?=htmlspecialchars($node['name'])?></span>
					<a href="https://plus.google.com/communities/<?=urlencode($node['node_id'])?>" target="_blank" class="ws_btn ws_tooltip" data-title="Community link" style="font-size: 13px; color: #fd79a8;"><i class="fa fa-external-link fa-external-link-alt"></i></a>
				</td>
				<td><?=htmlspecialchars($node['category'])?></td>
...
<div style="margin-bottom: 20px; margin-right: 20px; text-align: center;">
	<button class="ws_btn ws_bg_danger" type="button" data-load-modal="google_add_community" data-parameter-account_id="<?=$accountId?>"><i class="fa fa-plus"></i> Add new community</button>
	<button class="ws_btn" type="button" data-modal-close="true">Close</button>
</div>

<script>
	fsCode.modalWidth('<?=$mn?>' , 50)
	...
	$("#proModal<?=$mn?> #sub_menu2 > .activate_btn").click(function()
	{
		...
		$("#proModal<?=$mn?> #google-communities-list-tbl tr[data-id=\"" + dataId + "\"] .node_chckbx")

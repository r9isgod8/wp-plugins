...
$accountInf = DB::DB()->get_row("SELECT * FROM ".DB::table('accounts')." WHERE id='{$accountId}' AND driver='google' AND (user_id='{$userId}' OR is_public=1) " , ARRAY_A);
	...
	$("#proModal<?=$mn?> .community_select").change(function()
	{
		...
		fsCode.ajax('google_community_get_cats' , {'id': community, 'account_id': '<?=$accountId?>'}, function(res)
		{
			...
			if( res['cats'].length > 0 )
			{
				...
				$("#proModal<?=$mn?> .categories_area").slideDown(200);
			}
			else
			{

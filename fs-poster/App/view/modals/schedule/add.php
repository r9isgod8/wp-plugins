...
$accounts = DB::DB()->get_results(
	DB::DB()->prepare("
		SELECT tb2.*, tb1.filter_type, tb1.categories, (SELECT GROUP_CONCAT(`name`) FROM ".DB::DB()->base_prefix."terms WHERE FIND_IN_SET(term_id,tb1.categories) ) AS categories_name,'account' AS node_type FROM ".DB::table('account_status')." tb1
...
$activeNodes = DB::DB()->get_results(
	DB::DB()->prepare("
	SELECT tb2.*, tb1.filter_type, tb1.categories, (SELECT GROUP_CONCAT(`name`) FROM ".DB::DB()->base_prefix."terms WHERE FIND_IN_SET(term_id,tb1.categories) ) AS categories_name FROM ".DB::table('account_node_status')." tb1
...
<div style="padding: 10px 0 20px; margin-left: 40px; margin-right: 40px; display: flex; flex-direction: column;">
	...
	<div style="width: 100%; margin-top: 15px;">
		...
		<div>
			...
			<div data-oppen-tab-id="basic">
				...
				<div class="fs_form_group">
					<label><?=esc_html__('Name:' , 'fs-poster')?> <span class="ws_tooltip" data-title="You can add multiple Schedules. Please fill the name field because of confusions."><i class="fa fa-info-circle"></i></span></label>
					<input type="text" class="ws_form_element2 title" placeholder="<?=esc_html__('Name' , 'fs-poster')?>" style="width: 500px;">
				</div>
				...
				<div class="fs_form_group" style="width: 300px;">
					<label>Order posts by: <span class="ws_tooltip" data-title="Method for selecting posts."><i class="fa fa-info-circle"></i></span></label>
			...
			<div data-oppen-tab-id="filters" style="display: none; position: relative;">
				...
				<div class="fs_form_group">
					<label>By shared time of posts:</label>
					<select class="ws_form_element post_date_filter" onchange="fsRecalculatePostCount();" style="width: 300px;">
				...
				<div class="fs_form_group">
					<label>By post type: <span class="ws_tooltip" data-title="You can select new post types in [ FS Poster > Settings > Share post types ]."><i class="fa fa-info-circle"></i></span></label>
					<select class="ws_form_element post_type_filter" onchange="fsRecalculatePostCount();" style="width: 300px;">
						<?php
				...
				<div class="fs_form_group">
					<label>By post category and tag:</label>
					<select class="ws_form_element category_filter" onchange="fsRecalculatePostCount();" style="width: 300px;" data-placeholder="<?=esc_html__('Not  selected' , 'fs-poster')?>">
						<option>- All -</option>
						<?php
						...
						print '<optgroup label="Custom categories"></optgroup>';
						...
						foreach( $otherTaxonomies AS $categ )
						{
							...
							{
								print '</optgroup><optgroup label="' . htmlspecialchars( $categ->taxonomy ) . '">';
								$saveLastTaxonomyName = $categ->taxonomy;
							}

							print '<option value="' . htmlspecialchars($categ->term_id) . '">' . htmlspecialchars($categ->name) . '</option>';
						}
						print $saveLastTaxonomyName ? '</optgroup>' : '';
				...
				<div style="display: flex">
					...
					<div style="padding: 3px 15px;">
						<div class="fs_share_box_items" id="share_box1">
							...
							{
								$coverPhoto = Helper::profilePic($nodeInf);
								...
								<div class="fs_share_box_node" data-tab="<?=$nodeInf['driver']?>">
									...
									<div class="node_img"><img src="<?=$coverPhoto?>" onerror="$(this).attr('src', '<?=Helper::assets('images/no-photo.png')?>');"></div>
									<div class="node_label" style="width: 100%;">
										<div>
											<?=esc_html($nodeInf['name']);?>
											<a href="<?=Helper::profileLink($nodeInf)?>" target="_blank" class="ws_btn" title="Profile link" style="font-size: 13px; color: #fd79a8;"><i class="fa fa-external-link fa-external-link-alt"></i></a>
										</div>
		...
		<div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #FFAB00;">
			...
			<span style="float: right; background: #cccccc; padding: 5px 10px; border-radius: 3px;">Posts matching your filters: <b id="matched_posts_count">...</b></span>
		</div>
	</div>
</div>
...
<script>
	...
	jQuery(document).ready(function()
	{
		...
		$("#proModal<?=$mn?> .saveScheduleBtn").click(function()
		{
			...
			if( $('#matched_posts_count').text().trim() === '0' )
			{
				fsCode.toast("<?=esc_html__('No posts matched your filters!' , 'fs-poster')?>" , 'danger');
				return false;
			}
			...
			{
				fsCode.toast("<?php print esc_html__('No account or coummunications are selected!' , 'fs-poster')?>" , 'danger');
				return false;
			}
		...
		{
			$(this).closest('.fs_share_box_node').slideUp(200 , function()
			{
				$(this).remove();
			});
		});
	...
	{
		var post_date_filter    = $('#proModal<?=$mn?> .post_date_filter').val(),
			post_type_filter    = $('#proModal<?=$mn?> .post_type_filter').val(),
			category_filter     = $('#proModal<?=$mn?> .category_filter').val();

		fsCode.ajax('calcualte_post_count', {
			post_date_filter: post_date_filter,
			post_type_filter: post_type_filter,
			category_filter: category_filter
		}, function (result)
		{
			$('#matched_posts_count').text( result['count'] );
		});
	}
</script>
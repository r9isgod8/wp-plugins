...
<div style="width: 100%; margin-top: 47px; display: flex; justify-content: center; align-items: center;">
	<div class="ok_logo"><img src="<?=Helper::assets('images/ok.svg')?>"></div>
	<div style="width: 45%;">
...
<script>
	...
	function compleateOperation( status, errorMsg )
	{
		if( status )
		{
			...
			$('#fs_account_supports .fs_social_network_div[data-setting="ok"]').click();
		}
		else
		{
	...
	{
		var openURL = "<?=site_url().'/?ok_app_redirect='?>" + $("#appIdSelect").val() + '&proxy=' + $("#proxyInput").val();
		if( $("#appIdSelect>:selected").attr('data-standart') == '1' )
		{
			openURL = "<?=Helper::standartAppRedirectURL('ok')?>&proxy=" + $("#proxyInput").val();
		}

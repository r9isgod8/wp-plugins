...
class FacebookApi
{
	...
	public static function authorizeFbUser( $appId , $accessToken , $proxy)
	{
		...
		$checkLoginRegistered = DB::DB()->get_row( DB::DB()->prepare( "SELECT * FROM ".DB::table('accounts')." WHERE user_id=%d AND driver='fb' AND profile_id=%s" , [get_current_user_id() ,$meId] ) , ARRAY_A );
		...
		else
		{
			...
			DB::DB()->delete( DB::table('account_access_tokens')  , ['account_id' => $fbAccId , 'app_id' => $appId] );
		...
		{
			$accountsList = self::cmd('/me/accounts', 'GET' , $accessToken , ['fields' => 'access_token,category,name,id', 'limit' => 500] , $proxy );
			...
			{
				$accountsList = self::cmd('/me/accounts', 'GET' , $accessToken , ['fields' => 'access_token,category,name,id', 'limit' => '3'] , $proxy );
			}
	...
	{
		$insights = self::cmd('/' . $postId , 'GET' , $accessToken , [

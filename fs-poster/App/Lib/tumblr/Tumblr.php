...
namespace FSPoster\App\Lib\tumblr;
...
class Tumblr
{
	...
	public static function authorizeTumblrUser( $appId , $consumerKey, $consumerSecret , $accessToken , $accessTokenSecret , $proxy )
	{
		...
		{
			Helper::response(false , esc_html($e->getMessage()));
		}

		$username = $me->user->name;

		$checkLoginRegistered = DB::fetch('accounts' , ['user_id' => get_current_user_id() , 'driver' => 'tumblr', 'username' => $username]);
	...
	public static function getLoginURL($appId)
	{
		...
		$client = new Client($consumerKey, $consumerSecret , null, null , $_SESSION['fs_proxy_save']);
	...
	public static function getAccessToken( )
	{
		...
		$client = new Client($consumerKey, $consumerSecret , $_SESSION['tmp_oauth_token'] , $_SESSION['tmp_oauth_token_secret'] , $proxy);

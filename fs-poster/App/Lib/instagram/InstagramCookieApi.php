...
class InstagramCookieApi
{
	...
	public function uploadPhoto( $photo , $caption )
	{
		...
		try
		{
			$photoDetails = new PhotoDetails( $photo->getFile() );
		}
		catch (\Exception $e)
		{
			return [
	...
	public function uploadVide( $video , $caption )
	{
		...
		try
		{
			$videoDetails = new VideoDetails( $video->getFile() );
		}
		catch (\Exception $e)
		{
			return [
		...
		$videoThumbnail = new InstagramThumbnail( $videoDetails->getFilename( ) );
	...
	{
		$photo = new InstagramPhoto( $photo, ['targetFeed' => \InstagramAPI\Constants::FEED_STORY] );

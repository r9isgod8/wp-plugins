...
class FSCodeUpdater
{
	...
	public function push_update( $transient )
	{
		...
		if(
			$remote && version_compare( Helper::getVersion(), $remote->version, '<' )
		)
		{
			$res = new \stdClass();
			$res->slug = $this->plugin_slug;
	...
	private function get_transient()
	{
		...
		if( !$remote )
		{
			$remote = wp_remote_get( $this->update_url . '?act=check_update&domain=' . site_url() . '&purchase_code=' . $this->purchase_code);
	...
	{
		if(
			isset($_GET['fscode_check_for_update']) && $_GET['fscode_check_for_update'] == '1' &&
			isset($_GET['plugin']) && is_string($_GET['plugin']) && $_GET['plugin'] == $this->plugin_slug &&
			isset($_GET['_wpnonce']) && is_string( $_GET['_wpnonce'] ) && wp_verify_nonce($_GET['_wpnonce'], 'fscode_check_for_update_' . $this->plugin_slug)
		)
		{
			delete_transient( 'fscode_upgrade_' . $this->plugin_slug );
		}
